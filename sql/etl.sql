CREATE OR REPLACE PROCEDURE create_all_tables() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE clear_all_tables() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE call_all_procedures() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION log_message(message TEXT) RETURNS VOID LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE data_extraction(file_paths TEXT) LANGUAGE plpgsql AS $$
DECLARE
    file_path TEXT;
BEGIN
...
CREATE OR REPLACE PROCEDURE data_mapping() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE data_cleansing() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE normalize_data() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE populate_product_dimension() LANGUAGE plpgsql AS $$
DECLARE
	p_record RECORD;
...
CREATE OR REPLACE PROCEDURE create_product_dimension() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION handle_product_price_change() RETURNS TRIGGER AS $$
DECLARE
    next_id INTEGER;
BEGIN
...
BEFORE
UPDATE OF price_each ON product_dimension
FOR EACH ROW EXECUTE FUNCTION handle_product_price_change();

-- Price change function remains the same

CREATE OR REPLACE FUNCTION change_product_price (in p_product_name VARCHAR, in p_price_each DECIMAL) RETURNS VOID LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION insert_new_product(in p_product_name VARCHAR, in p_price_each DECIMAL) RETURNS VOID LANGUAGE plpgsql AS $$
DECLARE
    next_pk_id INTEGER;
    next_pid_id INTEGER;
BEGIN
...
CREATE OR REPLACE PROCEDURE create_time_dimension() AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE populate_location_dimension() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE create_location_dimension() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE create_final_fact_table() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE create_sales_data_cube() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE move_and_append_data() LANGUAGE plpgsql AS $$
BEGIN
...
CREATE OR REPLACE PROCEDURE public.extract_grains_loc(IN grain INTEGER, IN top_node VARCHAR) LANGUAGE 'plpgsql' AS $$
...
CREATE OR REPLACE PROCEDURE public.extract_grains_time(IN grain integer, IN top_node text) LANGUAGE 'plpgsql' AS $$
DECLARE
...
CREATE OR REPLACE PROCEDURE slice_cube(IN top_node_time TEXT, IN top_node_loc TEXT, IN time_grain INTEGER DEFAULT NULL, IN loc_grain INTEGER DEFAULT NULL) LANGUAGE plpgsql AS $$
BEGIN
    DELETE FROM sliced_cube;
    ...
    ELSIF (top_node_time IS NOT NULL AND top_node_loc IS NULL) THEN
        CALL extract_grains_time(time_grain, top_node_time);
    ...
    ELSIF (top_node_time IS NULL AND top_node_loc IS NOT NULL) THEN
        CALL extract_grains_loc(loc_grain, top_node_loc);
        INSERT INTO sliced_cube
            SELECT
                product_id,
                time_id,
